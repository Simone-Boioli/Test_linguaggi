...
public class ModificaProdotto extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		String nuovoPrezzo = request.getParameter("prezzo");
		...
		for(int i = 0; i < len; i++){
			...
			if(s.equals(codice)){ // abbiamo trovato il prodotto
				NodeList prez = ((Element)articolo).getElementsByTagName("prezzo"); //cerco il tag prezzo
				Node p = prez.item(0);
				
				Node testo = p.getFirstChild();
				
				//modifico il valore del nodo text
				testo.setNodeValue(nuovoPrezzo);
			}
		}
		...
		out.println("<body>");		
			out.println("<fieldset class=\"OPERAZIONE\">");
				out.println("<legend> Operazione </legend>");
					...
					out.println("Prodotto Modificato CORRETTAMENTE");
