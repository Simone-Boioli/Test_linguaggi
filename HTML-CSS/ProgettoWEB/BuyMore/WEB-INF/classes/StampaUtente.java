...
public class StampaUtente extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		String idUt = request.getParameter("idutente");
		...
		NodeList list = ((Element)child).getElementsByTagName("idUtente");
		...
		out.println("<body>");		
			out.println("<fieldset class=\"OPERAZIONE_STAMPA\">");
				out.println("<legend> Operazione </legend>");
				
				for(int i = 0; i < len; i++){
					Node testo = list.item(i).getFirstChild();
					
					if(testo.getNodeValue().equals(idUt)){
						Node u = testo.getParentNode().getParentNode();
					
						for(u = u.getFirstChild(); u != null; u = u.getNextSibling()){
							out.println(u.getNodeName() + " = " + u.getFirstChild().getNodeValue() + "<br>");
						}
