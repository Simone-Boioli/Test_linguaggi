...
public class RimuoviProdotto extends HttpServlet{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException{
		...
		NodeList list =((Element)child).getElementsByTagName("prodotto"); //lista di tutti prodotti
		...
		for(int i = 0; i < len; i++){
			...
			String s = c.getFirstChild().getNodeValue();
			...
			if(s.equals(codice)){ // abbiamo trovato il prodotto
				...
				prod = c.getParentNode().getParentNode();
				root = c.getParentNode().getParentNode().getParentNode(); // ci troviamo a magazzino
				
				rimosso = root.removeChild(prod);
...
out.println("<!-- HTML 5 -->");
		...
		out.println("<body>");		
			out.println("<fieldset class=\"OPERAZIONE\">");
				...
				if(rimosso != null){
					out.println("Prodotto Rimosso CORRETTAMENTE");
				}else{
					out.println("ERRORE - PRODOTTO NON RIMOSSO");
				}
